...
const Button:React.FC<ButtonProps> = ({className, type, onClick, disabled, children}) => {
    return (
        ...
        type={type} onClick={onClick} disabled={disabled} className={`${className} px-5 py-2 flex justify-center items-center font-[600] bg-gradient-to-r from-[#e0942e] to-[#fc4545] rounded-lg ${disabled ? "opacity-70" : "hover:-translate-x-0.5 hover:-translate-y-0.5" }`}>
            {children}
        </motion.button>
    )
}
